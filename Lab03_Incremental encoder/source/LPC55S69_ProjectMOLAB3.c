...
int main(void) {
	...
	while(1) {
		for(int i=0; i<6;i++) {
			if(mode && i==counter){
				sprintf(sbuff, " %s [%03d] (%03d)", MenuItems[i + offset].name, MenuItems[i + offset].val, MenuItems[i + offset].max);
			}
			else{
				offset = 6 % counter;
				sprintf(sbuff, " %s  %03d        ", MenuItems[i + offset].name, MenuItems[i + offset].val);
